...
public class Main
{
	...
	public static void main(String[] args)
	{
		...
		String fileName = "A1base.txt";
		Scanner input = null;
	
		// class for our auditorium
		// if we cannot find the file, exit the program
		Auditorium Theater = null;
		try
		{
			input = new Scanner(new File(fileName));
			Theater = new Auditorium(input);
			Theater.display();
		}
		...
		mainMenu(userIn, Theater);
		
		
		userIn.close();
		Theater.printReport(fileName);
		System.exit(0);
	}
	...
	private static void mainMenu(Scanner userIn, Auditorium Theater)
	{
		// our main menu loop
		int choice = 0;
		do 
